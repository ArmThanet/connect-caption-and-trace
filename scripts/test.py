...
def build_vocab(imgs, params):
    ...
    for img in imgs:
        for sent in img['caption']:
            ...
            # Join the list of words into a string for printing
            sent_str = ' '.join(words)
            
            print("Original Caption:", sent_str)
    ...
    # Print top 10 bad words and their counts
    cw = sorted([(count, w) for w, count in counts.items() if count <= count_thr], reverse=True)
    print('Top 10 bad words and their counts:')
    print('\n'.join(map(str, cw[:10])))
    ...
    for i,img in enumerate(imgs):
        ...
        for j,s in enumerate(img['final_captions']):
            ...
            for k,w in enumerate(s):
                if k < max_length:
                    # Encode the word into its corresponding index using wtoi
                    if w in wtoi:
                        Li[j,k] = wtoi[w]
                    else:
                        # If the word is not in the vocabulary, encode it as UNK
                        Li[j,k] = wtoi['UNK']
...
def main(params):
    ...
    for i, img in enumerate(imgs):
        ...
        # Check if 'split' is present in img
        if 'split' in img:
            jimg['split'] = img['split']
        else:
            jimg['split'] = 'val'  # Provide a default value if 'split' is not present
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--images_root', default='/mnt/c/Users/thane/Desktop/@Final/connect-caption-and-trace/scripts', help='root location in which images are stored, to be prepended to file_path in input json')
